...
- 修复的旧时间行数；
- 当前提示信息；
- 每个对话的成功点、下一批起点、稳定区终点和剩余条数；
- App 重启后仍存在的失败、空结果和失效批次；
- 失败批次的手动重试入口。

## 派生摘要召回

每日与月度摘要会持久保存 `sourceMemoryIdsJson`。当派生摘要与它覆盖的原子记忆同时进入候选集合时，召回器保留摘要并移除被覆盖的原子项，避免同一事实在一轮上下文中重复输入。旧归档会在 v29→v30 迁移时从兼容字段回填来源 ID。

## P1 状态

P1 的用户可配置批次、保护区、分支级 checkpoint、失败重试、失效重建、完整时间语义、派生摘要互斥和持久 UI 队列均已实现。后续上下文统一与更细的 token 预算属于 P2。
