...
object ExamStudyPlan {
    ...
    val studyHabitReference = """
        ...
        - 2026-07-03 用户昨天肚子不舒服、喝了药，又熬夜到 4 点，12 点才起来；状态不是完全不能学，但不能完成太累的任务。当天按“病后 + 睡眠债恢复日”处理：下午慢启动，保留不背单词 120 个和 1 个低压专业课入口，最多加 1 个轻复盘/错题小块；不要安排硬背诵、难题、长时间听课，也不要把昨天漏掉的任务整包追补。
    ...
    val dailyPlans: Map<LocalDate, DailyStudyPlan> = listOf(
        ...
        daily("2026-07-02", "法理2 + 刑法1", "法理学第 2 章：错题回看、框架整理、第一轮背诵", "刑法学第 1 章：听课抓犯罪论总入口", "刑法学第 1 章：做 10-15 题并标错因", dailyLongSentenceTaskTitle, "睡前 10 分钟整理明日最小任务"),
        daily("2026-07-03", "病后恢复日：12点起的低负荷版", "起床后先吃清淡、喝热水、洗漱，整理桌面 10 分钟；今天不追上午时长", "不背单词 120 个（6 组）：拆成 3 次完成，每次 2 组，中间必须离桌休息", "法理学第 2 章：只回看错题和目录框架 30 分钟，不背完整表述、不补难题", "饭后散步或慢走 15-20 分钟，等困劲过去再回桌面", "睡前 10 分钟写明天第一步，23:30 前结束学习入口"),
        daily("2026-07-04", "轻启动：刑法1入口 + 法理2小封口", "上午先观察身体状态，热水、清淡早饭、整理桌面 10 分钟后再开始", "刑法学第 1 章：听众合法硕刑法课程 30-40 分钟，只抓犯罪论总入口", "刑法学第 1 章：做 5 题并标错因，不追求题量", "法理学第 2 章：用 10 分钟口头复述框架，卡住就看目录提示", dailyVocabularyTaskTitle, "晚上不加新课，只做轻收尾和提前睡"),
        daily("2026-07-05", "法理3 + 民法1轻启动", "法理学第 3 章：目录式回忆并补框架", "民法第 1 章：听众合法硕民法课程 30-45 分钟，抓主体和民事法律关系入口", "民法第 1 章：只做 5-8 题建立题感", "散步 15 分钟"),
        daily("2026-07-06", "刑法1封口 + 法理回背", "刑法学第 1 章：补 8-10 题并整理错因", "法理第 1-3 章：目录式回背 20 分钟", "背诵：法理第 2-3 章关键词，不追求一口气背完", dailyVocabularyTaskTitle, "散步或拉伸 20 分钟"),
        daily("2026-07-07", "第一周复盘与容错", dailyVocabularyTaskTitle, "回看法理第 1-3 章错题", "回看刑法第 1 章和民法第 1 章错题", "补一个本周漏掉的小任务，只补最小一块", "好好吃一顿饭，散步 20 分钟"),
        ...
        daily("2026-07-09", "法理6 + 民法3", "法理学第 6 章：目录式回忆并补框架", "民法第 3 章：看课并做 10-15 题", "背诵：刑法第 1-3 章关键词", "画民法第 1-3 章框架", dailyLongSentenceTaskTitle),
    ...
    fun todaySchedule(date: LocalDate = LocalDate.now()): List<StudyScheduleBlock> {
        ...
        if (date == LocalDate.of(2026, 7, 3)) {
            return listOf(
                ...
                StudyScheduleBlock("13:20-14:00", "单词 1", "不背单词 40 个（2 组），只做识别和复习，不加长难句。"),
                StudyScheduleBlock("14:20-15:00", "法理入口", "法理学第 2 章只回看错题和目录框架，不背完整表述、不补难题。"),
                StudyScheduleBlock("15:00-15:25", "离桌缓冲", "散步、热水或拉伸，确认肚子舒服再继续。"),
