...
object LivingJudgmentModelPlanner {
    ...
    fun buildPrompt(input: LivingJudgmentModelInput): String = buildString {
        ...
        appendLine("行动实现 Action 只执行判断结果：MESSAGE, WAIT, TOOL_USE, SET_ALARM, WRITE_DIARY, SCHEDULE_NEXT_PERCEPTION, READ, ASK_USER, PASS。WRITE_DIARY 只表示后台心迹，不是正式日记；正式日记只通过 write_lulu_journal 工具保存。记忆提取不作为模型动作，辞海和聊天达到阈值后自动总结入向量记忆。")
        appendLine("状态生成在行动后发生，只生成 mood/bodyState/mindState/relationship/innerThought。innerThought 必须是角色第一人称没说出口的心理活动。")
        appendLine("辞海保存挂心任务、工具写入的正式日记、后台心迹和沉淀；正式日记记录角色真实感受和没说出口的想法，必须来自 write_lulu_journal 工具。挂心任务展示事件、目标、下次感知；不要把 belief、traitMotive、situationalMotive 当成用户可见状态栏。")
