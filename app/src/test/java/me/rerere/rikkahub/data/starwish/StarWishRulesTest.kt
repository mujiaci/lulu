...
class StarWishRulesTest {
    ...
    fun builtInScrollsExposeTwentyCompleteSoloAndInteractionPrompts() {
        ...
        StarWishRules.scrolls.forEach { scroll ->
            assertTrue("Scroll title should not be blank", scroll.title.isNotBlank())
    ...
    private fun assertPromptComplete(prompt: String, label: String) {
        listOf("服装", "饰品", "背景", "姿势", "表情", "光影", "画风", "画质").forEach { phrase ->
            assertTrue("$label should include $phrase", prompt.contains(phrase))
        }
        assertTrue("$label should name Lulu", prompt.contains("露露"))
        assertTrue(
            "$label should preserve Lulu as a male companion",
            prompt.contains("男生") || prompt.contains("恋人"),
        )
    }
